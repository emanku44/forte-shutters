...
function QuotesTab({ quotes, onStatusChange }) {
  ...
  return (
    <>
      ...
      {filtered.map(q => (
        <div key={q.id} className="adm-quote-card">
          <div className="adm-quote-top">
            <div>
              <div className="adm-quote-name">{q.customer_name}</div>
              <div style={{ fontSize: '0.75rem', color: '#8A9470', marginTop: 2 }}>{new Date(q.created_at).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: '2-digit' })}</div>
            </div>
            <select
              value={q.status || 'new'}
              onChange={e => onStatusChange(q.id, e.target.value)}
              style={{ ...c.inp, width: 'auto', padding: '5px 8px', fontSize: '0.78rem', color: statusColor[q.status || 'new'], borderColor: statusColor[q.status || 'new'] + '55' }}
            >
          ...
          <div className="adm-quote-meta">
            <div className="adm-quote-meta-item"><span className="ml">Product </span><span className="mv">{q.product_type || '—'}</span></div>
            <div className="adm-quote-meta-item"><span className="ml">Location </span><span className="mv">{q.location || '—'}</span></div>
            <div className="adm-quote-meta-item"><span className="ml">Motor </span><span className="mv">{q.motor || '—'}</span></div>
            <div className="adm-quote-meta-item">
              <span className="ml">Range </span>
              <span className="mv" style={{ color: '#C8960C' }}>
                {q.estimated_low ? `${(q.estimated_low/1000).toFixed(0)}k–${(q.estimated_high/1000).toFixed(0)}k` : '—'}
              </span>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            {q.customer_phone && (
              <a href={`https://wa.me/${q.customer_phone.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer"
                style={{ background: 'rgba(37,211,102,0.1)', color: '#25D366', border: '1px solid rgba(37,211,102,0.2)', borderRadius: 2, padding: '5px 12px', fontSize: '0.78rem', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 5 }}>
                💬 {q.customer_phone}
              </a>
            )}
            {q.customer_email && (
              <a href={`mailto:${q.customer_email}`}
                style={{ background: 'rgba(200,150,12,0.08)', color: '#C8960C', border: '1px solid rgba(200,150,12,0.2)', borderRadius: 2, padding: '5px 12px', fontSize: '0.78rem', textDecoration: 'none' }}>
                ✉ {q.customer_email}
              </a>
            )}
          </div>
          ...
          {q.final_notes && (
            <div style={{ marginTop: '0.5rem', fontSize: '0.78rem', color: '#8A9470', background: 'rgba(255,255,255,0.02)', borderRadius: 2, padding: '6px 8px' }}>
              📝 {q.final_notes}
            </div>
          )}
        </div>
